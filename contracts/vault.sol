 ...
 * Vault Contract
 * 
 * This contract is use for token vault and auto farming . Allows owner to store money into the vault and auto farmming token in target protocol.
 * 
 * Interface: 
 *  - Deposit
 *      Allow owner to deposit money into vault and select the target farming place.
 *  - Withdraw
 *      Allow owner to withdraw money from vault .
 *  - Settings
 *      Allow owner to setting the (support_token_list | target_token_farm | owner_replace)
 *
 */
